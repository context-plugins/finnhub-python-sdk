...
| Root package | `finnhub` |
| Distribution name | `finnhub` |
...
Alternatively, scope it — `with FinnhubClient(...) as client:` closes the pool on exit.
...
Alternatively, scope it — `async with AsyncFinnhubClient(...) as client:` closes the pool on exit.

`AsyncClient` (`finnhub/async_client.py`) mirrors `Client` method for method, each endpoint method a coroutine. It takes the same keywords, except that each client accepts only its own transport and — where the **Async Type** column differs — only its own flavor.

`Client` and `AsyncClient` are aliases of `FinnhubClient` and `AsyncFinnhubClient` — the names tracebacks and `repr()` show; all four import from the root.
...
Every operation is a method on the client. Every constructor argument is optional and keyword-only. Sources: `finnhub/client.py`, `finnhub/async_client.py`:
...
| --- | --- | --- |
| `HttpClient` | `finnhub.core` | protocol — `send(request: HttpRequest) -> HttpResponse` · `close()` |
| `AsyncHttpClient` | `finnhub.core` | protocol — `async send(request: HttpRequest) -> HttpResponse` · `async aclose()` |
...
- **Case A — typed error.** The operation documents at least one error status, so `finnhub/errors/` declares a union alias over the bodies those statuses map to — `RawError` is always its last arm, for any undocumented status — and `.error` is annotated with that alias. Narrow it with `isinstance`. The operation blocks name the alias and the status each arm maps from.
- **Case B — raw error.** The operation documents no error status; `.error` is `RawError` (`finnhub/core/results.py`): `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse`.

Core runtime types (`finnhub/core/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
| --- | --- | --- |
| `ApiError` — raised by every parsed call; `.error` is always `RawError` (no Case A alias in this SDK) | `error: E` · `status_code: int` · `response: HttpResponse` | `finnhub/core/exceptions.py` |
| `ApiResult[T, E]` — returned by every raw call; the `Success[T] \| Failure[E]` union | `payload: T` (on `Success`) · `error: E` (on `Failure`) · `response: HttpResponse` (on both) | `finnhub/core/results.py` |
| `RawError` | `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse` | `finnhub/core/results.py` |
...
```python
from finnhub.core import ApiError, RawError
...
## Operations — by controller (1 page, 117 operations)
...
| **A trailing `request_options`** — keyword-only and optional, for per-call overrides such as a timeout or extra headers; every signature ends with it | here (`finnhub/core/request_options.py`) |
...
| --- | --- |
| Client (root) | `finnhub` |
| Models | `finnhub.models` |
| Core runtime (`ApiError`, `ApiResult`, `RawError`, …) | `finnhub.core` |
...
**One environment, one server** (`finnhub/server/server_config.py`). The spec declares a single environment, so no `environment` keyword exists; the base URL and its override point:
